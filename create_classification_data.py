...
class Dataset():
    ...
    train_InputExamples=None,test_InputExamples=None,train_features=None,test_features=None):
        self.name = name
        self.train = train
        self.test = test
...
#%%
#*************ASTD-Unbalanced************
df_ASTD_UN = pd.read_csv('Datasets\\ASTD-master\\data\\Tweets.txt',sep='\t',header=None)

DATA_COLUMN = 'text'
LABEL_COLUMN = 'label'
df_ASTD_UN.columns = [DATA_COLUMN, LABEL_COLUMN]

df_ASTD_UN[LABEL_COLUMN] = df_ASTD_UN[LABEL_COLUMN].apply(lambda x: 0 if (x=='NEG') else x)
...
data_ASTD_UN = Dataset("ASTD-Unbalanced",train_ASTD_UN,test_ASTD_UN,label_list_ASTD_UN)
all_datasets.append(data_ASTD_UN)
#%%
#*************ASTD-Dahou-Balanced************

df_ASTD_B = pd.read_csv('Datasets\\Dahou\\data_csv_balanced\\ASTD-balanced-not-linked.csv',sep=',',header=0)
...
data_ASTD_B = Dataset("ASTD-Dahou-Balanced",train_ASTD_B,test_ASTD_B,label_list_ASTD_B)
all_datasets.append(data_ASTD_B)

#%%
#*************ArSenTD-LEV************
df_ArSenTD = pd.read_csv('Datasets\\ArSenTD-LEV\\ArSenTD-LEV-processed-no-emojis2.csv',sep=',',header=0)

df_ArSenTD.columns = [DATA_COLUMN, LABEL_COLUMN]

df_ArSenTD[LABEL_COLUMN] = df_ArSenTD[LABEL_COLUMN].apply(lambda x: 0 if (x=='very_negative') else x)
df_ArSenTD[LABEL_COLUMN] = df_ArSenTD[LABEL_COLUMN].apply(lambda x: 1 if (x=='negative') else x)
...
(d_train, y_train, d_test, y_test) = labr_helper.get_train_test(klass="2", balanced="unbalanced")
